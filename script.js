...
// ===== UNIVERSAL EMAIL & PHONE REDIRECT =====
(() => {
  const EMAIL = "anatomypmu@gmail.com";
  const PHONE = "+17574079859";

  // Усі кнопки або лінки, де є "email" у href, відкривають вкладку написання листа
  document.querySelectorAll('a[href*="mailto:"], a[href*="email"], button[data-action="email"]').forEach(a => {
  ...
  // Усі кнопки або лінки, де є "tel" або "call", відкривають дзвінок
  document.querySelectorAll('a[href*="tel:"], a[href*="call"], button[data-action="call"]').forEach(a => {
    a.addEventListener("click", e => {
      e.preventDefault();

...
// FAQ: закрити всі питання за замовчуванням
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('.acc-item').forEach(function (item) {
    // якщо це <details>, забираємо атрибут open
...
// ===========================
// BROWS TRANSFORMATIONS CAROUSEL
// ===========================
document.addEventListener('DOMContentLoaded', function () {
  ...
  carousels.forEach(function (carousel) {
    ...
    // Ініціалізація: показуємо тільки перший слайд
    ...
    carousel.addEventListener('touchend', function (e) {
      ...
      // невеликий поріг, щоб не ловити випадкові торкання
      if (Math.abs(diff) > 40) {
        if (diff < 0) {
...
// Script — Brows Transformations Carousel
document.addEventListener('DOMContentLoaded', function () {
  const carousels = document.querySelectorAll('[data-carousel]');
  if (!carousels.length) return;
